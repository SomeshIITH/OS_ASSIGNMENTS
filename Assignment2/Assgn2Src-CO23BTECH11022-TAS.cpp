...
atomic_flag TAS_LOCK = ATOMIC_FLAG_INIT;               //initializing TAS_LOCK


void lock_TAS(){                                      //locking the critical section ,emory_order_acquire-> Ensures no reordering of subsequent operations
    while(TAS_LOCK.test_and_set(memory_order_acquire)){
        if(Sudoku_Is_Valid==false)return;             //Early Termination step
    }        
}

void unlock_TAS(){                                    //unlocking the critical section,memory_order_release-> Ensures no reordering of previous operations
...
void* check_sudoku(void *arguement){
    ...
    while(Sudoku_Is_Valid && (C < (3*N)) ){                                  //Total number of tasks is 3N, so if C is greater than equal to 3N then all tasks are done   

        if(Sudoku_Is_Valid==false) return NULL;                                        // Early termination if Sudoku is already invalid
        ...
        if (Sudoku_Is_Valid==false) {                                              // Another thread already found invalidity
            unlock_TAS();                                                    // Release lock immediately
            break;
        }
        ...
        if(C >= 3*N){                               //Edge Case :if all tasks are done then and some thread entered inside Waiting Section , then just Give them entry in CS and exit them without any validation check
            ...
            unlock_TAS();                            //Deadlock Prevention : Directly breaking Edge case thread acquire the lock and thus other thread waiting in WAITING SECTION got stuck, so unlock it before exiting
        ...
        C_value = C;                                 //fetching the value of C , And then increment it by taskInc  , can be done by inbuilt fetch_add also. EX- C_value = C.fetch_add(taskInc);  //fetching and adding taskInc to C
        C+=taskInc;
        ...
        unlock_TAS();                                     //Signalling to unlock CS so that other thread can enter
        ...
        for(int i=start;i<end;i++){
            if (!Sudoku_Is_Valid) break;     // Stop immediately if Sudoku is invalid
...
int main(){
    ...
    auto total_time = duration_cast<microseconds>(ENDING_TIME - BASE_START_TIME);   //give final-start time in microseconds
    ...
    printf("Average time to enter CS: %f microseconds\n", Total_Time_To_Enter_CS/Total_CS_entry);
    printf("Average time to exit CS: %f microseconds\n", Total_Time_To_Exit_CS/Total_CS_entry);
    printf("Worst time to enter CS: %f microseconds\n", Worst_CS_Entry_time);
    printf("Worst time to exit CS: %f microseconds\n", Worst_CS_Exit_time);
