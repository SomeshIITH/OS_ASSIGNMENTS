...
atomic<int> CAS_LOCK(0);               //initializing CAS_LOCK to 0(unlock)


void lock_CAS(){     
    int expected = 0;                                 //locking the critical section ,emory_order_acquire-> Ensures no reordering of subsequent operations
    while(!CAS_LOCK.compare_exchange_strong(expected, 1, memory_order_acquire)){
        expected = 0;
    }       
}

void unlock_CAS(){                                    //unlocking the critical section,memory_order_release-> Ensures no reordering of previous operations
    CAS_LOCK.store(0,memory_order_release);  
}
...
void* check_sudoku(void *arguement){
    ...
    while((Sudoku_Is_Valid==true) && C< (3*N)){                                  //Total number of tasks is 3N, so if C is greater than equal to 3N then all tasks are done   
        ...
        if(Sudoku_Is_Valid==false) break;  
                                              // Early termination if Sudoku is already invalid
        ...
        if(Sudoku_Is_Valid==false) {                                              // Another thread already found invalidity
            unlock_CAS();                                                    // Release lock immediately
            break;                                    
        }
        ...
        if(C >= 3*N){                               //Edge Case :if all tasks are done then and some thread entered inside Waiting Section , then just Give them entry in CS and exit them without any validation check
            ...
            unlock_CAS();                            //Deadlock Prevention : Directly breaking Edge case thread acquire the lock and thus other thread waiting in WAITING SECTION got stuck, so unlock it before exiting
        ...
        unlock_CAS();                                     //Signalling to unlock CS so that other thread can enter
