...
int main(){
    ...
    //m2 for Spliiting
    for(int i=0;i<K1;i++){
        rowsplit[i].start=i*P1;
        i==K1-1 ? rowsplit[i].end=N-1 : rowsplit[i].end=(i+1)*P1-1;
        rowsplit[i].starttime = starting_time;
        rowsplit[i].tid=i;
    }

    for(int i=0;i<K2;i++){
        colsplit[i].start=i*P2;
        i==K2-1 ? colsplit[i].end=N-1 : colsplit[i].end=(i+1)*P2-1;
        colsplit[i].starttime = starting_time;
        colsplit[i].tid=i+K1;
    }

    for(int i=0;i<K3;i++){  
        blocksplit[i].start=i*P3;
        i==K3-1 ? blocksplit[i].end=N-1 : blocksplit[i].end=(i+1)*P3-1;
        blocksplit[i].tid=i+K1+K2;
        blocksplit[i].starttime = starting_time;
    }
